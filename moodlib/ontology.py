...
def query_instructions(text: str) -> str:
    ...
    return (
        ...
        "Two of these are routinely confused, and getting them wrong is the "
        "single most common way a playlist goes soft:\n"
        "  power = force, drive, weight. joyful_activation = cheerfulness, "
        "bounce, fun. They are independent. A hard techno record is high power "
        "and low joy; a pop anthem is the reverse.\n"
        "  A request tied to physical activity — workout, running, dancing — "
        "also needs a steady beat: give danceability real weight there, or "
        "powerful-but-arrhythmic music (an orchestral climax, a doom-metal "
        "dirge) matches on force alone.\n"
        "  tension is what separates INTENSE from merely upbeat. \"High "
        "intensity\", \"hard\", \"aggressive\", \"furious\", \"heavy\", "
        "\"relentless\" all mean high power AND high tension — give tension real "
        "weight for those, or the query also matches every cheerful club track "
        "with a big kick drum. Do not reach for joyful_activation to express "
        "intensity: a high joy target actively pulls in party and pop records.\n"
        ...
        "(tension/sadness).\n\n"
        "contexts = when the request names or implies an occasion — a workout, "
        "sleep, a party, a drive, studying — always list it. It is how a track "
        "tagged for the opposite occasion gets ruled out.\n"
        ...
        "  These are alternatives, not a description. List several ONLY when the "
        "request genuinely offers a choice (\"jungle or breakbeat\"). When one "
        "genre word MODIFIES another — \"ambient drum and bass\", \"jazzy "
        "house\", \"dark techno\", \"acoustic folk\" — the request names ONE "
        "thing: list only the head noun (drum and bass, house, techno) and "
        "express the modifier through the axes and emotions instead. Listing the "
        "modifier as a second style asks for either one, and the modifier "
        "usually wins: \"ambient drum and bass\" then returns ambient.\n"
        "  When the request names a genre, the mood targets describe variation "
        "WITHIN it, not the genre's absolute position. Chill drum and bass is "
        "still drum and bass — do not set an energy so low that nothing in the "
        "genre could reach it.\n"
